...
public class TutorRegisterForm extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        requestsRef = FirebaseDatabase.getInstance().getReference("registrationRequests").child("tutors");
    ...
    private boolean validateInputs() {
        ...
        if (TextUtils.isEmpty(degreeInput.getText().toString().trim())) {
            degreeInput.setError("Degree is required");
            isValid = false;
        }
        String courses = coursesInput.getText().toString().trim();
        if (TextUtils.isEmpty(courses) || courses.split(",").length == 0) {
            coursesInput.setError("List at least one course");
            isValid = false;
        }
